...
(function () {
    ...
    function ensureTableScrollWrap(containerSelector) {
        ...
        if (!root) return;
        var cardBodies = root.querySelectorAll('.card-body-padded');
        cardBodies.forEach(function (cardBody) {
            var tables = cardBody.querySelectorAll('table.data-display-table');
            tables.forEach(function (table) {
                if (table.parentNode && (table.parentNode.classList.contains('po-table-scroll-wrap') || table.parentNode.classList.contains('bo-table-scroll-wrap'))) return;
                if (!table.parentNode) return;
                var wrap = document.createElement('div');
                wrap.className = PAGE_MODE === 'admin' ? 'bo-table-scroll-wrap' : 'po-table-scroll-wrap';
    ...
    function injectPrintStyles() {
        ...
        s.textContent =
            ...
            '.bo-table-scroll-wrap{overflow:visible;width:100%}' +
            '.bo-table-scroll-wrap table.data-display-table{width:100%}' +
            ...
            '#bo-print-root{display:block!important}body.bo-printing #bo-print-root{position:static;width:100%}}';
    ...
    function renderRetailerTable(filter) {
        ...
        tbody.innerHTML = rows.map(function (o, i) {
            return '<tr data-bo="' + o.code + '" class="bo-data-row">' +
                '<td>' + (i + 1) + '</td>' +
                '<td>' + o.dateCreated + '</td>' +
                '<td><span class="bo-code-text">' + o.poCode + '</span></td>' +
                '<td>' + o.entity + '</td>' +
                '<td>' + (o.items ? o.items.length : 0) + '</td>' +
                '<td><span class="status-pill-badge ' + o.statusClass + '">' + o.status + '</span></td>' +
                '<td>' + buildActionMenu(o.code) + '</td></tr>';
        }).join('');
        if (footer) footer.textContent = 'Showing ' + rows.length + ' of ' + all.length + ' entries — sorted newest first';
    }
    ...
    function buildPrintReportHtml(order) {
        var total = boTotal(order);
        var rows = (order.items || []).map(function (it) {
            var qty = it.backOrder != null ? it.backOrder : it.qty;
            var lineTotal = it.total != null ? it.total : (Number(qty) || 0) * (Number(it.cost) || 0);
            return '<tr><td>' + formatMoney(qty) + '</td><td>' + it.unit + '</td>' +
                '<td>' + it.name + (it.note ? ' (' + it.note + ')' : '') + '</td>' +
                '<td style="text-align:right;">' + formatMoney(it.cost) + '</td>' +
                '<td style="text-align:right;">' + formatMoney(lineTotal) + '</td></tr>';
        }).join('');
        if (!rows) {
            rows = '<tr><td colspan="5" style="text-align:center;color:#888;">No back ordered items.</td></tr>';
        }
        return '<div class="bo-receipt-sheet">' +
            '<div class="bo-receipt-header"><h1>Kreezby Bakeshop</h1><p>Back Order Report</p></div>' +
            '<div class="bo-receipt-meta">' +
            ...
            '</div>' +
            '<table class="bo-receipt-table"><thead><tr>' +
            '<th>Qty</th><th>Unit</th><th>Item</th><th>Cost</th><th>Total</th>' +
            '</tr></thead><tbody>' + rows + '</tbody>' +
            '<tfoot><tr><td colspan="4" style="text-align:right;">Grand Total</td>' +
            '<td style="text-align:right;">' + formatMoney(total) + '</td></tr></tfoot></table>' +
            '<p class="bo-receipt-verify">Verification: ' + order.status + ' — Printed ' + new Date().toLocaleString() + '</p>' +
            '</div>';
    }
    ...
    function printReport(boCode) {
        ...
        if (!root) return;
        root.innerHTML = buildPrintReportHtml(order);
    ...
    function switchTab(tabName) {
        ...
        var panel = document.getElementById('bo-tab-' + tabName);
